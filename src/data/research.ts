...
export const ARTICLES: Article[] = [
  // --- RUNTIME SECURITY ---
  {
    ...
    featured: true,
    sections: STANDARD_SECTIONS.map(s => s.id === "best-practices" ? { ...s, content: [s.content[0], `Monitor and log every request. For a complete understanding of how this fits into your broader architecture, read our complete guide to prompt injection. To see how Krixai automates this protection, explore our product.`] } : s),
  ...
  },
  {
    ...
    sections: STANDARD_SECTIONS.map(s => s.id === "key-takeaways" ? { ...s, content: [s.content[0], `Every AI request should be inspected before it reaches the model. This is the minimum viable security posture for LLM applications. Learn more about AI security fundamentals.`] } : s),
  ...
  },
  {
    ...
    sections: STANDARD_SECTIONS.map(s => s.id === "engineering-implementation" ? { ...s, content: [s.content[0], `The pipeline should execute sequentially. Each stage operates independently and can be configured per workspace. For a comprehensive overview of how runtime layers protect agents, review our AI security guide.`] } : s),
  ...
  },
  {
    ...
    sections: STANDARD_SECTIONS.map(s => s.id === "technical-explanation" ? { ...s, content: [s.content[0], `This means a carefully constructed user input can override, modify, or extract the system prompt. Understanding these mechanisms is crucial to understanding prompt injection attacks as a whole.`] } : s),
  ...
  },
  {
    ...
    sections: STANDARD_SECTIONS.map(s => s.id === "technical-explanation" ? { ...s, content: [s.content[0], `This means a carefully constructed user input can override, modify, or extract the system prompt. Understanding these evasion techniques is critical for defending against prompt injection attacks.`] } : s),
  ...
  },
  {
    ...
    sections: STANDARD_SECTIONS.map(s => s.id === "key-takeaways" ? { ...s, content: [s.content[0], `Every AI request should be inspected before it reaches the model. When securing data access, RBAC forms a core part of an enterprise RAG security architecture.`] } : s),
  ...
  // --- RESEARCH ---
  {
    ...
    sections: STANDARD_SECTIONS.map(s => s.id === "problem" ? { ...s, content: [s.content[0], `The attack surface is no longer limited to network layers and authentication boundaries. It now extends into the semantic layer. To understand these threats holistically, review our AI security fundamentals, and specifically our guide to prompt injection (LLM01).`] } : s),
