...
export const ARTICLES: Article[] = [
  // --- RUNTIME SECURITY ---
  {
    ...
    featured: true,
    sections: STANDARD_SECTIONS.map(s => s.id === "best-practices" ? { ...s, content: [s.content[0], `Monitor and log every request. For a complete understanding of how this fits into your broader architecture, read our <a href="/prompt-injection" className="text-blue-400 hover:underline">complete guide to prompt injection</a>. To see how <a href="/product" className="text-blue-400 hover:underline font-medium">Krixai</a> automates this protection, explore our product.`] } : s),
  ...
  },
  {
    ...
    sections: STANDARD_SECTIONS.map(s => s.id === "key-takeaways" ? { ...s, content: [s.content[0], `Every AI request should be inspected before it reaches the model. This is the minimum viable security posture for LLM applications. Learn more about <a href="/ai-security" className="text-blue-400 hover:underline">AI security fundamentals</a>.`] } : s),
  ...
  },
  {
    ...
    sections: STANDARD_SECTIONS.map(s => s.id === "engineering-implementation" ? { ...s, content: [s.content[0], `The pipeline should execute sequentially. Each stage operates independently and can be configured per workspace. For a comprehensive overview of how runtime layers protect agents, review our <a href="/ai-security" className="text-blue-400 hover:underline">AI security guide</a>.`] } : s),
  ...
  // --- THREAT INTELLIGENCE ---
  {
    ...
    sections: STANDARD_SECTIONS.map(s => s.id === "problem" ? { ...s, content: [s.content[0], `The attack surface is no longer limited to network layers and authentication boundaries. It now extends into the semantic layer. For context on these external data threats, refer to our <a href="/indirect-prompt-injection" className="text-blue-400 hover:underline">guide to indirect prompt injection</a>, or explore our <a href="/rag-security" className="text-blue-400 hover:underline">RAG security architecture</a>.`] } : s),
  ...
  },
  {
    ...
    sections: STANDARD_SECTIONS.map(s => s.id === "technical-explanation" ? { ...s, content: [s.content[0], `This means a carefully constructed user input can override, modify, or extract the system prompt. Understanding these mechanisms is crucial to <a href="/prompt-injection" className="text-blue-400 hover:underline">understanding prompt injection attacks</a> as a whole.`] } : s),
  ...
  },
  {
    ...
    sections: STANDARD_SECTIONS.map(s => s.id === "technical-explanation" ? { ...s, content: [s.content[0], `This means a carefully constructed user input can override, modify, or extract the system prompt. Understanding these evasion techniques is critical for defending against <a href="/prompt-injection" className="text-blue-400 hover:underline">prompt injection attacks</a>.`] } : s),
  ...
  // --- ENGINEERING ---
  {
    ...
    sections: STANDARD_SECTIONS.map(s => s.id === "engineering-implementation" ? { ...s, content: [s.content[0], `The pipeline should execute sequentially: Unicode normalization → prompt injection detection → sensitive data redaction → policy evaluation → risk scoring. Each stage operates independently and can be configured per workspace. <a href="/product" className="text-blue-400 hover:underline font-medium">Krixai</a> provides this exact architecture out-of-the-box.`] } : s),
  ...
  },
  {
    ...
    sections: STANDARD_SECTIONS.map(s => s.id === "key-takeaways" ? { ...s, content: [s.content[0], `Every AI request should be inspected before it reaches the model. When securing data access, RBAC forms a core part of an enterprise <a href="/rag-security" className="text-blue-400 hover:underline">RAG security architecture</a>.`] } : s),
  ...
  // --- RESEARCH ---
  {
    ...
    sections: STANDARD_SECTIONS.map(s => s.id === "problem" ? { ...s, content: [s.content[0], `The attack surface is no longer limited to network layers and authentication boundaries. It now extends into the semantic layer. To understand these threats holistically, review our <a href="/ai-security" className="text-blue-400 hover:underline">AI security fundamentals</a>, and specifically our <a href="/prompt-injection" className="text-blue-400 hover:underline">guide to prompt injection</a> (LLM01).`] } : s),
